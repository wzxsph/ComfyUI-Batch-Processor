...
class ABComparisonWidget(ctk.CTkFrame):
    ...
    def draw(self):
        ...
        if self.img_a_pil and split_x_in_img > 0:
            img_a_resized = self.img_a_pil.resize(
        ...
        if self.img_a_pil and self.img_b_pil:
            ...
            self.canvas.create_line(
                line_x, off_y, line_x, off_y + new_h, fill="white", width=2
            )
...
class ModernComfyUIApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        lbl_lang = ctk.CTkLabel(
            lang_frame,
            text=t("language_selector_label"),
            font=ctk.CTkFont(family="Microsoft YaHei", size=11),
        )
        lbl_lang.pack(side="left")

        lang_options = [
            self.localizer.get_language_name("en"),
            self.localizer.get_language_name("zh"),
        ]
        ...
        self.lbl_paths = ctk.CTkLabel(
            self.sidebar_frame,
            text=t("directories_workflow"),
            font=ctk.CTkFont(family="Microsoft YaHei", size=14),
        )
        self.lbl_paths.grid(row=4, column=0, padx=20, pady=(10, 0), sticky="w")

        self.ent_folder_a = ctk.CTkEntry(
            self.sidebar_frame,
            placeholder_text=t("input_folder_placeholder"),
            font=ctk.CTkFont(family="Microsoft YaHei", size=13),
        )
        self.ent_folder_a.grid(row=5, column=0, padx=20, pady=(5, 5), sticky="ew")
        self.btn_folder_a = ctk.CTkButton(
            self.sidebar_frame,
            text=t("btn_select_input"),
            command=lambda: self.select_folder(self.ent_folder_a),
        ...
        self.ent_folder_b = ctk.CTkEntry(
            self.sidebar_frame,
            placeholder_text=t("output_folder_placeholder"),
            font=ctk.CTkFont(family="Microsoft YaHei", size=13),
        )
        self.ent_folder_b.grid(row=7, column=0, padx=20, pady=(5, 5), sticky="ew")
        self.btn_folder_b = ctk.CTkButton(
            self.sidebar_frame,
            text=t("btn_select_output"),
            command=lambda: self.select_folder(self.ent_folder_b),
            fg_color="transparent",
            border_width=1,
            height=32,
            font=ctk.CTkFont(family="Microsoft YaHei", size=14),
        ...
        self.ent_json = ctk.CTkEntry(
            self.sidebar_frame,
            placeholder_text=t("api_json_placeholder"),
            font=ctk.CTkFont(family="Microsoft YaHei", size=13),
        )
        self.ent_json.grid(row=9, column=0, padx=20, pady=(5, 5), sticky="ew")
        self.btn_json = ctk.CTkButton(
            self.sidebar_frame,
            text=t("btn_select_json"),
        ...
        self.lbl_load_id = ctk.CTkLabel(
            self.node_frame,
            text=t("load_image_id"),
            font=ctk.CTkFont(family="Microsoft YaHei", size=13),
        )
        ...
        self.lbl_save_id = ctk.CTkLabel(
            self.node_frame,
            text=t("save_image_id"),
            font=ctk.CTkFont(family="Microsoft YaHei", size=13),
        )
        ...
        self.btn_start = ctk.CTkButton(
            ...
            text=t("btn_start"),
            height=50,
            font=ctk.CTkFont(family="Microsoft YaHei", weight="bold", size=18),
        ...
        self.main_frame.grid_rowconfigure(
            3, weight=0
        )  # Log Area takes strict space when visible
        ...
        self.gallery_frame = ctk.CTkFrame(
            self.preview_container, fg_color="transparent"
        )
        ...
        self.lbl_gallery_status = ctk.CTkLabel(
            self.gallery_frame,
            text=t("gallery_status", count=0),
        ...
        self.btn_reload = ctk.CTkButton(
            ...
            border_width=1,
            font=ctk.CTkFont(family="Microsoft YaHei", size=13),
            command=self.load_gallery_from_disk,
        )
    ...
    def load_gallery_from_disk(self):
        ...
        self.gallery_images = sorted(
            [f for f in os.listdir(folder_b) if f.lower().endswith(valid_extensions)]
        )

        if self.gallery_images:
            if self.current_gallery_idx < 0 or self.current_gallery_idx >= len(
                self.gallery_images
            ):
    ...
    def update_gallery_view(self):
        ...
        self.lbl_gallery_status.configure(
            text=t(
                "gallery_image_info",
                current=self.current_gallery_idx + 1,
    ...
    def next_image(self):
        if self.gallery_images:
            self.current_gallery_idx = (self.current_gallery_idx + 1) % len(
                self.gallery_images
            )
            self.update_gallery_view()
    ...
    def start_processing(self):
        ...
        if not all([url, folder_a, folder_b, api_json, load_id, save_id]):
            messagebox.showwarning(
                t("msgbox_input_error_title"), t("msgbox_input_error_message")
            )
            return

        if not os.path.exists(folder_a) or not os.path.exists(api_json):
            messagebox.showerror(
                t("msgbox_path_error_title"), t("msgbox_path_error_message")
            )
            return
        ...
        threading.Thread(
            target=self.process_workflow_thread,
            args=(url, folder_a, folder_b, api_json, load_id, save_id, prefix),
            daemon=True,
        ).start()

    def process_workflow_thread(
        self, url, folder_a, folder_b, api_json, load_id, save_id, prefix=""
    ):
        try:
            ...
            valid_extensions = (".png", ".jpg", ".jpeg", ".webp")
            images_to_process = [
                f for f in os.listdir(folder_a) if f.lower().endswith(valid_extensions)
            ]
            ...
            for i, filename in enumerate(images_to_process, 1):
                ...
                if save_id in workflow and "inputs" in workflow[save_id]:
                    ...
                    workflow[save_id]["inputs"]["filename_prefix"] = (
                        f"{prefix}{name_without_ext}" if prefix else name_without_ext
                    )

                self.append_log(t("log_executing"))
                outputs = api.process_prompt_ws(
                    prompt=workflow,
                    status_callback=lambda val, max_val: self.update_progress(
                        val, max_val
                    ),
            ...
            messagebox.showerror(
                t("msgbox_error_title"), t("msgbox_error_message", error=str(e))
            )
